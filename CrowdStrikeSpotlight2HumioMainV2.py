...
def main():
    ...
    log_file = log_config['log_file'] + '_v' + version + '_' + str(log_time) +'.log'
    ...
    updated_timestamp = cs_config['updated_timestamp']
    sort = cs_config['sort']
    ...
    try:
        falcon = SpotlightVulnerabilities(client_id=clientID,client_secret=secret, base_url=base_url, proxy=proxies, user_agent=user_agent, timeout=timeout)
        if facets:
            logging.info(log_label + 'there is Facet data configured to be collected')
            response = falcon.query_vulnerabilities_combined(base_url=base_url, limit=limit, filter=spot_filter, proxy=proxies, facet=facets, sort=sort)
        else:
            logging.info(log_label + 'Facet data is not configured be collected')
            response = falcon.query_vulnerabilities_combined(base_url=base_url, limit=limit,filter=spot_filter, proxy=proxies, sort=sort)
    ...
    if ir_status_code.startswith('2'):
        logging.info(log_label + 'The initial API call appears to have been successful.')
        data = body['resources']
    ...
    if total_vul_ids == 0:
        ...
        logging.info(log_label + 'Preparing to send initial data collection to Humio')
    ...
    if total_vul_ids > int(limit):
        ...
        logging.info(log_label + 'Pagination has been identified as being needed')
    else:
        pagination_needed = False
        logging.info(log_label + 'Paginiation has not been identified as being needed')
    
    #while necessary make additional paginiation calls and extend vulnerability list
    while pagination_needed == True:
        try:
            if facets:
                logging.info(log_label + 'Facet data is configured to be collected')
                after_response = falcon.query_vulnerabilities_combined(base_url=base_url, limit=limit, filter=spot_filter, proxy=proxies, facet=facets, after=after_value, sort=sort )
            else:
                logging.info(log_label + 'Facet data is not configured to be collected')
                after_response = falcon.query_vulnerabilities_combined(base_url=base_url, limit=limit, filter=spot_filter, proxy=proxies, after=after_value, sort=sort)
                
        except Exception as e: 
            logging.error(log_label + 'Unable to contact the CrowdStrike API for pagination. The error returned was: ' + str(e))
        ...
        if after_status_code.startswith('2'):
            logging.info(log_label + 'A pagination API call appears to have been successful.')
            
        else:
            logging.error(log_label + 'A pagination API call appears to have failed.')
            connection_errors(logging, log_label, after_headers, after_body, after_status_code)
        
        after_data = after_body['resources']
        
        data.extend(after_data)
        logging.info(log_label + 'Current length of data dict:' + str(len(data)))
    ...
    logging.info(log_label + 'Vulnerability processing has completed. Total number of vulnerabilities identified: ' + str(total_vul_ids) + ' Total number of vulnerabilities processed: ' + str(total_vul_collected))
    sys.exit()
